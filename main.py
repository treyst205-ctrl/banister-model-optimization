...
#2. Initial conditions and solver
y0=[0.0,0.0]       
t_span=np.arange(1,201)     
t_eval=np.linspace(0,200,1000)
sol=solve_ivp(banister_model,(1,200),[0,0],t_eval=t_span,args=(tau_f,tau_a,k1,k2))
#true solution
F_sol=sol.y[0]
D_sol=sol.y[1]
P_sol=p0+F_sol-D_sol #performance

#exact day of peak performance
peak_index=np.argmax(P_sol)
peak_day=t_span[peak_index]
print("Peak performance at day ",peak_day)

#mock data generation
ut_array=np.array([u(t) for t in t_span]) #repeat weeklly cycle
np.random.seed(42) #important: keep initial random value, comment out this line to run different samples
...
initial_guess=[30.0,10.0,0.5,1.5,45.0] #inital guess for quasi-newton optimizer
bounds=[(20,70),(5,25),(0.1,5.0),(0.1,5.0),(40,60)] #guardrails: ensure a realistic answer
opt_result=minimize(objective_function,initial_guess,args=(t_span, p_calculated),bounds=bounds) #leave method as default L-BFGS-B
...
#repeat the Banister with our found parameters
...
plt.grid(True)
plt.show()
#end optmization plot

#setup the 30 day additional window
t_forecast=np.linspace(200,230,100) 
def u_future(t):
    return 0
F_now,D_now=sol_final.y[0][-1],sol_final.y[1][-1] #new IC's given the 200th day perforamnce
...
pafter_30d=p0_fit+sol_30d.y[0]-sol_30d.y[1]


#final forecast flotting
plt.figure(figsize=(10,6))
plt.plot(t_span, p_fitted_curve,color='blue',label='Numerical Best fit')
#future forecast (Days 200-230)
plt.plot(t_forecast,pafter_30d,color='red',label='Forecasted Prediction')
